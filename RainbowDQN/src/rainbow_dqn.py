...
from RainbowDQN.utils.config_manager import ConfigManager
import torch.serialization
torch.serialization.add_safe_globals([deque])
# ----------------------
# Setup and Configuration
# ----------------------
config_manager = ConfigManager('RainbowDQN/config/dqn_config.yaml')
...
def noisy_factorized(x):
    """
    Transformation f(x) = sign(x)*sqrt(|x|)
    wie im NoisyNet-Paper vorgeschlagen.
    """
...
class FactorizedNoisyLinear(nn.Module):
    def __init__(self, in_features, out_features, sigma_init=0.5):
        ...
        # Parameter-Matrizen für Mittelwerte (mu)
        self.weight_mu = nn.Parameter(torch.FloatTensor(out_features, in_features))
        self.bias_mu = nn.Parameter(torch.FloatTensor(out_features))

        # Parameter-Matrizen für Standardabweichung (sigma)
        self.weight_sigma = nn.Parameter(torch.FloatTensor(out_features, in_features))
        self.bias_sigma = nn.Parameter(torch.FloatTensor(out_features))

        # Zwischenspeicher für das aktuelle Rauschen
        # (kein Parameter, sondern nur Buffer)
    ...
    def reset_parameters(self):
        """
        Initialisiert die Parameter nach gängigem NoisyNet-Schema.
        """
        # Bound für das Uniform-Init
        bound = 1.0 / math.sqrt(self.in_features)
        
        self.weight_mu.data.uniform_(-bound, bound)
        self.bias_mu.data.uniform_(-bound, bound)
        
        # sigma (Standardabweichung) wird meist konstant gesetzt
        self.weight_sigma.data.fill_(self.sigma_init / math.sqrt(self.in_features))
        self.bias_sigma.data.fill_(self.sigma_init / math.sqrt(self.in_features))

    def reset_noise(self):
        """
        Zieht neues factorisiertes Rauschen für
        (in_features) und (out_features).
        """
        # Rauschen für Input- und Output-Dimension
        ...
        # Transformation f(x) = sign(x)*sqrt(|x|)
        epsilon_in = noisy_factorized(epsilon_in)
        epsilon_out = noisy_factorized(epsilon_out)
        
        # Outer product, damit wir (out_features, in_features) kriegen
        # => factorisiertes Rauschen
        self.epsilon_weight = epsilon_out.unsqueeze(1) * epsilon_in.unsqueeze(0)
        
        # Für die Bias-Terme: nur ein 1D-Vektor
        self.epsilon_bias = epsilon_out

    def forward(self, x):
        if self.training:
            # Im Training: Rauschen dazuaddieren
            # (ggf. hier reset_noise() für "Rauschen pro Forward-Pass")
            weight = self.weight_mu + self.weight_sigma * self.epsilon_weight
            bias = self.bias_mu + self.bias_sigma * self.epsilon_bias
        else:
            # In Evaluation: kein zusätzliches Rauschen
...
class NStepBuffer:
    ...
    def get_n_step_info(self):
        ...
        for idx, (_, _, reward, _, done) in enumerate(self.buffer):
            ...
            if done:
                break
        # The next state is the state in the last transition in the buffer.
        n_step_next_state = self.buffer[-1][3]
        # Done flag: if any transition was terminal, then we treat it as done.
        done_flag = self.buffer[-1][4]
        initial_state, initial_action = self.buffer[0][0], self.buffer[0][1]
        return initial_state, initial_action, R, n_step_next_state, done_flag
...
class DuelingNoisyDQN(nn.Module):
    def __init__(self, input_dim, output_dim, filename="model_weights.pth"):
        ...
        self.value_net_1 = FactorizedNoisyLinear(conv_output_size, lin1_output_size)
        self.value_net_2 = FactorizedNoisyLinear(lin1_output_size, 1)
            
        self.advantage_net_1 = FactorizedNoisyLinear(conv_output_size, lin1_output_size)
        self.advantage_net_2 = FactorizedNoisyLinear(lin1_output_size, output_dim)
...
def train_DQN(policy_net, target_net):
    ...
    lives = info.get("lives", None)
    n_step = 5  # for instance, a 3-step return
    ...
    for episode in range(start_episode, config['training']['num_episodes']):
        ...
        while not done and steps < config['training']['max_steps']:
            ...
            if current_lives is not None and current_lives < lives and not done:
                ...
                reward += fire_reward # Strafanpassung optional
                done = terminated or truncated
                current_lives = info.get("lives", current_lives)
            lives = current_lives
            ...
            n_step_buffer.append((state, reduced_action, reward, next_state_tensor, done))

            if n_step_buffer.is_full() or done:
                s, a, n_step_return, s_n, done_flag = n_step_buffer.get_n_step_info()
                replay_buffer.add((s, a, n_step_return, s_n, done_flag))
                # Remove the oldest transition to slide the window:
                n_step_buffer.pop_left()

            # Also, at episode end, clear any remaining transitions:
            if done:
                n_step_buffer.clear()
            ...
            if steps % 2 == 0 and len(replay_buffer) >= max(config['training']['batch_size'], 5000):
                ...
                rewards_n = torch.FloatTensor(rewards).to(device)
                next_states_n = torch.stack(next_states).to(device)
                ...
                with torch.no_grad():
                    ...
                    action_predicted = policy_net(next_states_n).max(1)[1].unsqueeze(1)
                    q_targets_next = target_net(next_states_n).gather(1, action_predicted).squeeze(1)
                    q_targets = rewards_n + (config['training']['gamma']**n_step) * q_targets_next * (1 - dones)
                ...
                TARGET_UPDATE_FREQUENCY = 10000 if episode > 1000 else (3000 if episode > 300 else 500)
        ...
        if episode % 10 == 0:
            ...
            torch.save(policy_net.state_dict(), "RainbowDQN/model_weights.pth")
